...
const drawCanvas = async (bpmnXML) => {
  ...
  debug(eventBus, elementRegistry, modeling);

}

// debug entry for cliHelper
const debug = async (eventBus, elementRegistry, modeling) => {
  // create Helper class
  ...
  bpmnJs.on('element.contextmenu', HIGH_PRIORITY, (elm) => {
    ...
    // 
    storeBO = getBusinessObject(elm.element);
    let bo = storeBO;
    console.log(bo);
    
    // storeSuitabilityScoreEl.value = bo.suitable ? bo.suitable : '';
    elSuitabilityScore.value = bo.suitable ? bo.suitable : '';
    elSuitabilityScore.focus();
    storeAnalysisDetailsElm = getExtensionElement(bo, 'qa:AnalysisDetails');
  ...
  elForm.addEventListener('submit', (event) => {
    ...
    // 
    const extensionElements = storeBO.extensionElements || moddle.create('bpmn:ExtensionElements');
    ...
    modeling.updateProperties(storeElTarget, {
      extensionElements,
